...
// Variables pour la publicité
            ...
            children: [
              ...
              thumb.isNotEmpty
                  ? CachedNetworkImage(
                      imageUrl: thumb,
                      fit: BoxFit.cover,
                      placeholder: (_, __) => Container(color: _colors.shimmerBase),
                      errorWidget: (_, __, ___) => _thumbFallback(post),
                    )
                  : _thumbFallback(post),
  ...
  String _thumbUrlFor(Post post) {
    if (post.dataType == PostDataType.VIDEO.name && post.thumbnail != null && post.thumbnail!.isNotEmpty) {
      return _optimizeImageUrl(post.thumbnail!);
    }
    if (post.images != null && post.images!.isNotEmpty) {
      return _optimizeImageUrl(post.images!.first);
    }
    if (post.url_media != null && post.url_media!.isNotEmpty) {
      return _optimizeImageUrl(post.url_media!);
    }
    return '';
  }

  Widget _thumbFallback(Post post) {
    ...
    return Container(
      ...
      child: Center(
        child: Icon(
          ...
          size: 48,
          color: _colors.textSecondary.withOpacity(0.5),
        ),
      ),
    );
  }
...
📞 Contactez le support si vous pensez qu'il s'agit d'une erreur.''');
  ...
  void _processLikeBackground(String userId, String postId, bool alreadyLiked) {
    ...
    coinProvider.sendLikeWithCoins(
      ...
      // Succès pièces : notifications + popularité supplémentaire
      if (!alreadyLiked) {
        try {
          await firestore.collection('Posts').doc(postId).update({
            'popularity': FieldValue.increment(3),
          ...
          if (userDoc.exists) {
            ...
            if (nowMicro - lastNotif >= twentyMin || lastNotif == 0) {
              ...
              if (targetUser?.oneIgnalUserid != null && targetUser!.oneIgnalUserid!.isNotEmpty) {
                await authProvider.sendNotification(
                  userIds: [targetUser.oneIgnalUserid!],
                  smallImage: authProvider.loginUserData.imageUrl ?? '',
                  send_user_id: userId,
