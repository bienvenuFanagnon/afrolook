...
class ListUsersEntrepriseChats extends StatefulWidget {
  const ListUsersEntrepriseChats({super.key});

  @override
  State<ListUsersEntrepriseChats> createState() => _ListUsersEntrepriseChatsState();
}

class _ListUsersEntrepriseChatsState extends State<ListUsersEntrepriseChats> {
                ...
                Expanded(
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        Text("@${chat.sender!.pseudo!}", style: TextStyle(fontSize: 16),),
                        SizedBox(height: 6,),
  ...
  Stream<List<Chat>> getAllChatsData() async* {
    ...
    var chatsStream = FirebaseFirestore.instance.collection('Chats')
        .where( 'entreprise_id', isEqualTo:  '${userProvider.entrepriseData.id}')
...
// Obtenez la liste des utilisateurs
    ...
    await for (var chatSnapshot in chatsStream) {
      ...
      for (var chatDoc in chatSnapshot.docs) {
        ...
        QuerySnapshot querySnapshotUser = await friendCollect.where("id",isEqualTo:chatDoc["sender_id"]).get();
        // Afficher la liste
                      ...
                      itemBuilder: (context, index){
                        return GestureDetector(
                          onTap: () async {
                            ...
                            Navigator.push(context, PageTransition(type: PageTransitionType.fade, child: EntrepriseMyChat(title: 'mon chat', chat: snapshot.data![index], post: snapshot.data![index]!.post!, isEntreprise: true,)));
