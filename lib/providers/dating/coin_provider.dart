...
class CoinProvider extends ChangeNotifier {
  ...
  CoinProvider({this.authProvider}) {
    _coinService = CoinService(authProvider: authProvider);
  }
  ...
  Future<bool> convertCoinsToXof(int amount) async {
    ...
    try {
      return await _coinService.convertCoinsToXofForCurrentUser(amount);
    } catch (e) {
      _error = e.toString();
      notifyListeners();
      return false;
  ...
  Future<bool> spendCoins({
    ...
    try {
      return await _coinService.spendCoinsForCurrentUser(
        amount: amount,
        type: type,
  ...
  Future<bool> hasEnoughCoins(int requiredAmount) async {
    return await _coinService.currentUserHasEnoughCoins(requiredAmount);
  }
