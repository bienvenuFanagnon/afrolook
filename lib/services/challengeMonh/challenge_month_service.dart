...
// lib/services/challenge_monh/challenge_month_service.dart
import 'package:cloud_firestore/cloud_firestore.dart';
import '../../models/model_data.dart';
import '../../providers/authProvider.dart';
...
class ChallengeMonthService {
  ...
  final String appDataId = 'XgkSxKc10vWsJJ2uBraT';
// lib/services/challenge_monh/challenge_month_service.dart

  /// Récupère les meilleurs posts du mois (pour affichage widget externe)
  /// Utilisé pour afficher le top 3 des posts avec le plus d'interactions
  Future<List<Post>> getTopPostsForMonth(DateTime month, {int limit = 3}) async {
    ...
    // 🔥 REQUÊTE : Récupère les posts triés par totalInteractions
    final query = await _firestore
        ...
        .orderBy('totalInteractions', descending: true)  // 🔥 TRI PAR INTERACTIONS
  ...
  // Récupérer la date de début des challenges
  ...
  /// 🔥 Récupérer les posts du mois avec TRI par totalInteractions DESC
  Future<List<Post>> getPostsForMonthBatch({
    ...
    // 🔥 REQUÊTE OPTIMISÉE : Tri direct par totalInteractions
    Query query = _firestore
        ...
        .where('created_at', isLessThan: endTs)
        .orderBy('totalInteractions', descending: true)  // 🔥 TRI PAR INTERACTIONS
        .orderBy('created_at', descending: true);       // Second tri en cas d'égalité
    ...
    for (var doc in snapshot.docs) {
      ...
      // Charger les données utilisateur/canal si nécessaire
  ...
  /// Annuler le gagnant pour un mois donné
  ...
  Future<void> payoutWinner({
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      transaction.set(_firestore.collection('TransactionSoldes').doc(transactionId), {
        'id': transactionId,
  ...
  /// Obtenir l'historique des validations (gagnants des mois passés)
  ...
  /// Obtenir les posts d'un mois pour l'admin (sans pagination, pour choisir le gagnant)
  Future<List<Post>> getTopPostsForMonthAdmin(DateTime month, {int limit = 10}) async {
    final startDate = await getChallengeStartDate();
    final monthStart = DateTime(month.year, month.month, 1);
    final monthEnd = DateTime(month.year, month.month + 1, 1);
    ...
    final query = await _firestore
        ...
        .where('created_at', isGreaterThanOrEqualTo: startTs)
        .where('created_at', isLessThan: endTs)
        .orderBy('totalInteractions', descending: true)
        .orderBy('created_at', descending: true)
        .limit(limit)
        .get();
