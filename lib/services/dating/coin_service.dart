...
class CoinService {
  ...
  Future<bool> buyCoins(CoinPackage package) async {
    try {
      ...
      printVm('📱 Achat de pièces pour l\'utilisateur: $userId');
      printVm('📦 Pack: ${package.name} - ${package.coinsAmount} pièces - ${package.priceXof} FCFA');
      ...
      if ((userData.votre_solde_principal ?? 0) < package.priceXof) {
        printVm('❌ Solde insuffisant: ${userData.votre_solde_principal} < ${package.priceXof}');
      ...
      return await _firestore.runTransaction((transaction) async {
        ...
        printVm('💰 Solde principal déduit: $newPrincipalBalance FCFA');
        ...
        printVm('✅ Transaction d\'achat créée: $transactionId');
        ...
        printVm('🎁 ${package.coinsAmount} pièces ajoutées. Nouveau solde: $newCoinsBalance');

        return true;
      });
    } catch (e) {
      printVm('❌ Erreur lors de l\'achat de pièces: $e');
      return false;
    }
  }
  ...
  Future<bool> spendCoins({
    ...
    try {
      if (userId != _currentUserId) {
        printVm('⚠️ Tentative de dépense pour un autre utilisateur');
      }
      ...
      if ((userData.coinsBalance ?? 0) < amount) {
        printVm('❌ Solde de pièces insuffisant: ${userData.coinsBalance} < $amount');
      ...
      return await _firestore.runTransaction((transaction) async {
        ...
        printVm('💰 $amount pièces déduites. Nouveau solde: $newCoinsBalance');
        ...
        printVm('✅ Transaction de dépense créée: $transactionId');

        return true;
      });
    } catch (e) {
      printVm('❌ Erreur lors de la dépense de pièces: $e');
      return false;
    }
  }
  ...
  Future<bool> creditCreator({
    ...
    try {
      printVm('📱 Crédit du créateur: $creatorId - $amount pièces');
      ...
      return await _firestore.runTransaction((transaction) async {
        ...
        if (walletQuery.docs.isEmpty) {
          ...
          printVm('✅ Nouveau wallet créé pour $creatorId');
        } else {
          ...
          printVm('💰 Wallet mis à jour: ${wallet.balanceCoins} → $newBalance');
        }
        ...
        printVm('✅ Transaction de crédit créée: $transactionId');

        return true;
      });
    } catch (e) {
      printVm('❌ Erreur lors du crédit du créateur: $e');
      return false;
    }
  }
  ...
  Future<bool> convertCoinsToXof({
    ...
    try {
      printVm('📱 Conversion de $amount pièces en FCFA pour $creatorId');
      ...
      printVm('💰 Taux de conversion: 100 pièces = $conversionRate FCFA');
      printVm('💰 Montant converti: $xofAmount FCFA');
      ...
      return await _firestore.runTransaction((transaction) async {
        ...
        if (wallet.balanceCoins < amount) {
          printVm('❌ Solde insuffisant: ${wallet.balanceCoins} < $amount');
          throw Exception('Solde de pièces insuffisant');
        }
        ...
        printVm('💰 Pièces déduites: ${wallet.balanceCoins} → $newBalance');
        ...
        printVm('✅ Demande de conversion créée: $conversionId');
        ...
        printVm('💰 $xofAmount FCFA ajoutés au solde gain');

        return true;
      });
    } catch (e) {
      printVm('❌ Erreur lors de la conversion: $e');
      return false;
    }
  }
  ...
  Future<List<UserCoinTransaction>> getUserTransactions(String userId) async {
    try {
      printVm('📱 Récupération des transactions pour $userId');
      ...
      printVm('✅ ${snapshot.docs.length} transactions trouvées');
      return snapshot.docs
          .map((doc) => UserCoinTransaction.fromJson(doc.data()))
          .toList();
    } catch (e) {
      printVm('❌ Erreur lors de la récupération des transactions: $e');
      return [];
    }
  }
  ...
  Future<CreatorCoinWallet?> getCreatorWallet(String creatorId) async {
    try {
      ...
      printVm('❌ Erreur récupération wallet: $e');
      return null;
    }
  }
  ...
  Future<bool> hasEnoughCoins(String userId, int requiredAmount) async {
    try {
      ...
      printVm('💰 Vérification solde pour $userId: ${userData.coinsBalance} >= $requiredAmount → $hasEnough');
      return hasEnough;
    } catch (e) {
      printVm('❌ Erreur vérification solde: $e');
      return false;
    }
  }
